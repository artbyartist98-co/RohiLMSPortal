...
export default function App() {
  ...
  const handlePushAllToSheets = async () => {
    ...
    setSyncingStatus('syncing');
    showToast('Uploading full secure database backup to Google Sheet tables...', 'info');
    ...
    try {
      ...
      localStorage.setItem('rohi_last_sync_time', nowStr);
      setSyncingStatus('success');
      showToast('Cloud database backup uploaded and organized into Sheet!', 'success');
    } catch (err) {
      console.warn('Google Sheets backup push failed:', err);
      setSyncingStatus('error');
      showToast('Error syncing full backup with Google Apps Script Web App.', 'err');
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      ...
      ════════════════════════════════════════════════════════ */}
      {voucherRecord && (
        <div className={`bg-slate-100 py-8 border-t border-gray-200 print:bg-white print:py-0 print:border-0 print:border-t-0 ${
          activeView === 'success' ? 'block' : 'hidden print:block'
        }`}>
          <div className="max-w-7xl mx-auto px-4 w-full">
            <div className="flex justify-between items-center mb-4 max-w-[1060px] mx-auto no-print">
